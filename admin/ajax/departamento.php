...
switch ($_GET["op"]) {
    ...
    case 'listar':
		...
		while ($reg=$rspta->fetch_object()) {
            ...
            "1"=>$reg->descripcion,
            "2"=>$reg->fechacreada,
            "3"=>'<button class="btn btn-primary btn-x" onclick="mostrar('.$reg->iddepartamento.')"><i class="fa fa-pencil"></i></button>'.' '.'<button class="btn btn-danger btn-x" onclick="desactivar('.$reg->iddepartamento.')"><i class="fa fa-close"></i></button>'
              );
		}
		$results=array(
